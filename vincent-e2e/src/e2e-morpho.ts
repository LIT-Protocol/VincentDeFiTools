...
(async () => {
  ...
  // ========================================
  console.log("🧪 Testing Morpho Tool - Complete Vault Workflow");
  console.log(
    "📋 Workflow: Deposit WETH → Withdraw WETH"
  );
  ...
  let initialVaultShares: ethers.BigNumber = ethers.BigNumber.from(0);

  // Get vault contract for state tracking
  console.log("🔍 Recording initial Morpho vault state...");
  const vaultContract = new ethers.Contract(
    NETWORK_CONFIG.wethVaultAddress,
    [
      // ERC4626 methods we need
      {
